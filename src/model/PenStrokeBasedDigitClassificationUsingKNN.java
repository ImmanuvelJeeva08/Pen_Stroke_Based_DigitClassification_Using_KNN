...
//        }
        ...
        List<double[]> trainX = new ArrayList<>();   // pen stroke movements
        List<String> trainY = new ArrayList<>();     // trueLabel
        List<double[]> testX = new ArrayList<>();    // pen stroke movements
        List<String> testY = new ArrayList<>();      // trueLabel
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter("src\\resultFile\\pendigits_result.txt"))) {
            for (int i = 0; i < predictions.size(); i++) {
                out.printf("%d %s %s %.6f%n", i, predictions.get(i), testY.get(i), accs.get(i));
            }
        }
